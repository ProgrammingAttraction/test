...
const Banneduser = () => {
  ...
  const fetchBannedUsers = async () => {
    try {
      const response = await axios.get(`${base_url}/admin/all-ban-users`,{  headers: {
  ...
  const handleStatusChange = async (userId, newStatus) => {
    try {
      await axios.put(`${base_url}/admin/users/${userId}/status`, { status: newStatus },{  headers: {
          'Authorization': `Bearer ${localStorage.getItem('genzz_token')}`
        }});
      fetchBannedUsers(); // Refresh the user list
    } catch (err) {
        ...
        <div className="">
          <div className="w-full p-4">
            <div className="flex justify-between items-center ">
              <h1 className="text-2xl font-semibold text-red-600 mb-6">Banned Users</h1>
            ...
            <div className="flex flex-wrap gap-4 mb-6 text-gray-700">
              ...
              <button 
                ...
                className="self-end ml-2 px-4 py-2 bg-cyan-600 text-white text-gray-700 rounded-[5px] hover:bg-cyan-700"
              >
                Reset Filters
              </button>
            </div>
              ...
              <table className="w-full border-collapse shadow-xl bg-white border-[1px] border-[#eee] overflow-hidden">
                <thead>
                  <tr className="bg-cyan-600 text-white">
                    ...
                    <th className="py-3 px-4 text-left">Ban Reason</th>
                  ...
                  {currentUsers.length > 0 ? (
                    currentUsers.map((user, index) => (
                      <tr key={index} className="border-b even:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-red-500 font-[600]">
                          <p>{user.reason || 'Not specified'}</p>
                             <p className="text-gray-600">        {new Date(user.createdAt).toLocaleDateString('EN', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
