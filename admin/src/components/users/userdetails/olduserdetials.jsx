...
const UserDetail = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      setLoading(true);
      const [userRes, financialRes, referralRes] = await Promise.all([
  ...
  // Calculate wagering information
      ...
      <div className="p-4 min-w-full">
        ...
        <section className="mb-[20px]">
          <div className="grid grid-cols-2 gap-4">
            <div className="border border-indigo-500 p-4 rounded-lg shadow bg-white">
            ...
            <div className="border border-orange-500 p-4 rounded-lg shadow bg-white">
              <span className="flex items-center text-lg font-medium text-black">
        ...
        {/* Action Buttons */}
        <div className="w-full grid grid-cols-2 md:grid-cols-5 gap-4 p-4 bg-white rounded-lg shadow mb-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{referralDetails?.totalReferrals || userDetail.referralCount || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Active Referrals:</span>
                <span className="font-medium">{referralDetails?.activeReferrals || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Deposited Referrals:</span>
                <span className="font-medium">{referralDetails?.depositedReferrals || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Referral Earnings:</span>
                <span className="font-medium">৳{referralDetails?.referralEarnings || userDetail.referralEarnings?.toFixed(2) || '0.00'}</span>
              </div>
            </div>

            {/* Referral Details Toggle Section */}
            {showReferralDetails && referralDetails && (
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left">Deposited</th>
                      ...
                      {referralDetails.referredUsers && referralDetails.referredUsers.length > 0 ? (
                        referralDetails.referredUsers.map((user, index) => (
                          <tr key={index} className="border-b">
                            ...
                            <td className="px-4 py-2">{formatDate(user.lastActive)}</td>
                            <td className="px-4 py-2">{user.hasDeposited ? 'Yes' : 'No'}</td>
                            <td className="px-4 py-2">৳{user.earnedAmount?.toFixed(2) || '0.00'}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan="6" className="px-4 py-2 text-center text-gray-500">No referred users found</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {userDetail.loginHistory && userDetail.loginHistory.length > 0 
                    ? formatDate(userDetail.loginHistory[userDetail.loginHistory.length - 1].timestamp) 
                    : 'Never'}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h3 className="text-xl font-semibold mb-4 pb-2 border-b border-gray-200 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-blue-500 hover:text-blue-700"
              >
                {showBonusDetails ? <FaChevronUp /> : <FaChevronDown />}
              </button>
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between">
            ...
            {/* Bonus Details Toggle Section */}
            {showBonusDetails && (
              <div className="mt-4 border-t pt-4">
                <h4 className="font-semibold mb-3">Bonus Activity Logs</h4>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead className="bg-gray-100">
                      <tr>
                      ...
                      {bonusActivityLogs.length > 0 ? (
                        bonusActivityLogs.map((log, index) => (
                          <tr key={index} className="border-b">
                            ...
                            <td className="px-4 py-2">৳{log.bonusAmount?.toFixed(2) || '0.00'}</td>
                            <td className="px-4 py-2">
                              {log.depositAmount > 0 ? `৳${log.depositAmount.toFixed(2)}` : "-"}
                            </td>
                            <td className="px-4 py-2">
                              {log.activatedAt ? formatDate(log.activatedAt) : "-"}
                            </td>
                            <td className="px-4 py-2">
                        ...
                        <tr>
                          <td colSpan="5" className="px-4 py-2 text-center text-gray-500">No bonus activity found</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
