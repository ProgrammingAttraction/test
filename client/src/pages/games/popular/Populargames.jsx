...
import controller_img from "../../../assets/famous.png";
...
const GameCard = ({ game, onPlayGame, onTryDemo, t }) => {
  ...
  return (
    <div className="relative group overflow-hidden rounded-[5px] md:rounded-xl bg-[#0a1920] shadow-lg transition-all duration-300 hover:shadow-xl hover:shadow-teal-900/30 hover:scale-[1.03] border border-gray-800">
      {/* Image */}
      <img
        src={game.imageUrl}
        alt={game.gameName}
  ...
  return (
    <div className="relative mx-[10px] mb-4 mt-3">
      <div className="relative">
        <input
          type="text"
          placeholder={t.searchPlaceholder || 'Search popular games...'}
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full bg-white border border-gray-200 rounded-lg px-10 py-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent backdrop-blur-sm"
        />
...
const ProviderTabs = ({ providers, selectedProvider, setSelectedProvider, t }) => {
  ...
  const scroll = (direction) => {
    if (scrollContainerRef.current) {
      ...
      // Update arrow visibility after a short delay to allow scrolling to complete
      setTimeout(() => {
        checkScrollPosition();
      }, 300);
    }
      ...
      >
        {providers.map(provider => (
          <button
            ...
            className={`flex-shrink-0 flex items-center cursor-pointer gap-2 px-4 py-2 md:py-1 border-[1px] rounded-[5px] text-sm font-medium transition-all ${
              selectedProvider === provider.providerName 
                ? 'bg-theme_color2 text-white shadow-lg border-theme_color2' 
          ...
          >
            ...
            <span className='text-[16px]'>{provider.providerName}</span>
          </button>
        ))}
      </div>
...
const PopularGamesContent = () => {
  ...
  // Use the GamesContext
  const { games, loading, error, refreshData } = useGames();
  const { userData } = useUser();
  const navigate = useNavigate();

  // Filter games to show only popular games (check categories array)
  const popularGames = games.filter(game => 
    game.categories && game.categories.some(cat => cat.toLowerCase() === 'জনপ্রিয়')
  );
  ...
  const filteredGames = popularGames.filter(game => {
    ...
    const matchesProvider = selectedProvider ? game.providerName === selectedProvider : true;
    
    return matchesSearch && matchesProvider;
  });

  // Get providers that actually have popular games
  const availableProviders = providers.filter(provider => 
    popularGames.some(game => game.providerName === provider.providerName)
  );

  // Load more games function with 2-second delay
  const loadMoreGames = () => {
  ...
  const handlePlayGame = async (game) => {
    try {
      ...
      // Send balance to Game API before starting the game
      await postBalanceToGameAPI(userData.balance, sessionId);

      // Now proceed with the game play
      // First check if game has lobby
      let lobbyData = null;
      if (game.has_lobby) {
        const lobbyResponse = await axios.get(`${API_BASE_URL}/api/games/games/lobby`, {
          params: {
            game_uuid: game.uuid,
            currency: "BDT"
          },
      ...
      // Check for the success response and URL for redirection
  ...
  const handleTryDemo = async (game) => {
    try {
      ...
      if (initResponse.data.success && initResponse.data.url) {
        // Navigate to game page with the URL in state
        navigate('/single-game', { 
          state: { 
            gameUrl: initResponse.data.url,
            gameName: game.gameName,
    ...
    <div className="container mx-auto pt-2 pb-3 sm:py-5">
      ...
      {visibleGames.length > 0 ? (
        ...
        <div className="text-center py-12 text-gray-400">
          <p>
            {searchTerm && selectedProvider 
              ? `${t.noGamesFound} "${searchTerm}" ${t.and} ${selectedProvider}` 
              : searchTerm 
                ? `${t.noGamesFound} "${searchTerm}"` 
                : `${t.noGamesFound} ${selectedProvider}`}
          </p>
          <button
            onClick={() => {
              setSearchTerm('');
      ...
      {!hasMoreGames && filteredGames.length > 0 && (
        <div className="text-center py-6 text-teal-400">
          <p>{t.allGamesLoaded || 'All popular games loaded'}</p>
        </div>
      )}
    </div>
  );
...
// Main Populargames Page Component
const Populargames = () => {
  ...
  return (
    <section className="min-h-screen font-anek bg-white to-gray-900">
