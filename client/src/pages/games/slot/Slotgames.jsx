  ...
  return (
    <div className="relative mx-[10px] mb-4 mt-3">
      <div className="relative">
        <input
          type="text"
          placeholder={t.searchPlaceholder || 'Search slot games...'}
          value={searchTerm}
      ...
      >
        {providers.map(provider => (
          <button
            ...
            className={`flex-shrink-0 flex items-center cursor-pointer borde-[1px] gap-2 px-4 py-2 md:py-1 rounded-[5px] text-sm font-medium transition-all ${
              selectedProvider === provider.providerName 
                ? 'bg-theme_color2 text-white shadow-lg' 
                : 'bg-gray-700 text-gray-700 border-gray-200'
            }`}
          >
            {provider.imageUrl && (
              <img 
...
const SlotGamesContent = () => {
  ...
  // Filter games to show only slot games (check categories array)
  const slotGames = games.filter(game => 
    game.categories && game.categories.some(cat => cat.toLowerCase() === 'স্লট গেম')
  );
  ...
  // Get providers that actually have slot games
  const availableProviders = providers.filter(provider => 
    slotGames.some(game => game.providerName === provider.providerName)
  );
    ...
    <div className="container mx-auto pt-2 pb-3 sm:py-5">
      ...
      {!hasMoreGames && filteredGames.length > 0 && (
        <div className="text-center py-6 text-teal-400">
          <p>{t.allGamesLoaded || 'All slot games loaded'}</p>
        </div>
      )}
    </div>
  );
