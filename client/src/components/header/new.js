...
const Header = ({ setShowPopup, setActiveLeftTab, showPopup, activeLeftTab }) => {
  ...
  const toggleBalanceDropdown = () => setBalanceDropdownOpen(!balanceDropdownOpen);
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .animate-dropdownFadeIn { animation: dropdownFadeIn 0.2s ease-out forwards; }
        ...
        .balance-btn .balance-amount {
          color: #ffffff;
          font-size: 13px;
          font-weight: 600;
          letter-spacing: 0.3px;
        }
        .balance-btn .balance-currency {
          color: #aaaacc;
          font-size: 12px;
          font-weight: 500;
        }
        .balance-btn .chevron-icon {
          color: #aaaacc;
          font-size: 16px;
          transition: transform 0.2s ease;
        }
        .balance-btn.open .chevron-icon {
        ...
        .balance-dropdown-item .item-icon {
          width: 32px;
          height: 32px;
          border-radius: 50%;
          display: flex;
        ...
        .balance-dropdown-item .item-icon.main {
          background: rgba(34,197,94,0.15);
          color: #22c55e;
        }
        .balance-dropdown-item .item-icon.bonus {
          background: rgba(168,85,247,0.15);
          color: #a855f7;
        }
        .balance-dropdown-item .item-label {
          color: #aaaacc;
          font-size: 12px;
          font-weight: 500;
        }
        .balance-dropdown-item .item-value {
          color: #ffffff;
          font-size: 14px;
          font-weight: 700;
          margin-top: 1px;
        ...
        .dropdown-action-btn.deposit {
          background: #22c55e;
          color: white;
        }
        .dropdown-action-btn.deposit:hover {
          background: #16a34a;
        }
        .dropdown-action-btn.withdraw {
          background: rgba(255,255,255,0.08);
          color: #ccccee;
          border: 1px solid rgba(255,255,255,0.1);
        }
        .dropdown-action-btn.withdraw:hover {
      ...
      <header className="bg-[#EEF0F8] sticky top-0 left-0 px-3 md:px-8 py-2 md:py-3 z-[100] flex items-center justify-between border-b border-gray-200">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          {!isAuthenticated ? (
            ...
            <>
              {/* ── Balance Button + Deposit Button + Dropdown ── */}
       <div className="relative inline-flex items-center border-[2px] border-gray-200 rounded-[10px] " ref={balanceDropdownRef}>
  {/* Balance pill button */}
  <button
    className={`flex items-center gap-2   transition-colors h-10 px-3  ${balanceDropdownOpen ? 'ring-1 ring-white/10' : ''}`}
    onClick={toggleBalanceDropdown}
  >
    <IoIosArrowDown className={`text-gray-800 transition-transform duration-200 ${balanceDropdownOpen ? 'rotate-180' : ''}`} />
    <div className="flex items-center gap-1 font-semibold text-gray-800">
      <span>{userData?.balance?.toFixed(0) ?? '0'}</span>
  ...
  <button
    className="flex items-center justify-center rounded-[10px] bg-theme_color2 text-white w-10 h-10  font-bold text-xl transition-colors"
    onClick={() => {
      setBalanceDropdownOpen(false);
      const isMobile = window.innerWidth < 768;
      if (isMobile) { navigate('/deposit'); }
      else { setActiveLeftTab(t.deposit || 'Deposit'); setShowPopup(true); }
    }}
  >
    +
  </button>
  ...
  {/* Dropdown */}
  {balanceDropdownOpen && (
    <div className="absolute top-[calc(100%+8px)] right-0 min-w-[200px] bg-white rounded-xl border-[1px] border-gray-200 p-5 shadow-2xl z-50 animate-in fade-in zoom-in duration-200 origin-top-left">
      
      {/* Main balance row */}
      <div className="flex items-start gap-4 mb-6">
        <div className="mt-1">
          {/* Custom Money Icon */}
         <LuWallet className='text-[22px] text-theme_color2'/>
        </div>
        <div className="flex flex-col">
          <span className="text-gray-700 text-[15px] font-medium leading-tight">Main balance</span>
          <span className="text-gray-700 text-xl font-bold mt-1">
            {userData?.balance?.toFixed(0) ?? '0'} <span className="text-lg">৳</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex items-start gap-4">
        <div className="mt-1">
         ...
         <svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          {/* Outer ring */}
          <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" className="text-theme_color2" />
          {/* Inner solid circle */}
          <circle cx="12" cy="12" r="5" fill="currentColor" className="text-theme_color2" />
          {/* Professional chip markings */}
          <path d="M12 2V5M12 19V22M2 12H5M19 12H22M5.6 5.6L7.8 7.8M16.2 16.2L18.4 18.4M18.4 5.6L16.2 7.8M7.8 16.2L5.6 18.4" 
                stroke="currentColor" 
                strokeWidth="1.5" 
                strokeLinecap="round" 
                className="text-theme_color2" />
        </svg>
        </div>
        <div className="flex flex-col">
          <span className="text-gray-700 text-[15px] font-medium leading-tight">Bonus Balance</span>
          <span className="text-gray-700 text-xl font-bold mt-1">
            {userData?.bonusBalance?.toFixed(0) ?? '0'} <span className="text-lg">৳</span>
          </span>
        </div>
      </div>
    </div>
